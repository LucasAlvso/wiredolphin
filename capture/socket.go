...
func (c *Capturer) processPacket(data []byte, ll *unix.SockaddrLinklayer) {
	...
	etherType, payload, ok := parser.DecodeLinkLayer(data, ll)
	if !ok {
		return
	}
...
func (c *Capturer) parseTCP(data []byte, pkt *stats.PacketInfo) {
	summary, err := parser.SummarizeTCP(data)
	if err != nil {
		return
	}

	pkt.TransportProto = summary.Proto
	pkt.SrcPort = summary.SrcPort
	pkt.DstPort = summary.DstPort
	pkt.TCPFlags = summary.Flags
	pkt.AppProto = summary.AppProto
	pkt.AppInfo = summary.AppInfo
}

// parseUDP parses a UDP packet
func (c *Capturer) parseUDP(data []byte, pkt *stats.PacketInfo) {
	summary, err := parser.SummarizeUDP(data)
	if err != nil {
		return
	}
